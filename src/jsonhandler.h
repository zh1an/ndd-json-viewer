...
constexpr auto flgBaseReader =
    rj::kParseEscapedApostropheFlag | rj::kParseNanAndInfFlag | rj::kParseNumbersAsStringsFlag;
...
class JsonHandler
{
  ...
  public:
    ...
    auto GetCompressedJson(const std::string &jsonText) -> Result;
    auto FormatJson(const std::string &jsonText, LE le, LF lf, char indentChar, unsigned indentLen) -> Result;
    auto ValidateJson(const std::string &jsonText) -> Result;

    template <unsigned format, typename Handler>
    auto ParseJson(const std::string &jsonText, rj::StringBuffer &sb, Handler &handler) -> Result;
};

template <unsigned flgBase, typename Handler>
inline auto JsonHandler::ParseJson(const std::string &jsonText, rj::StringBuffer &sb, Handler &handler) -> Result
{
    Result retVal{};
